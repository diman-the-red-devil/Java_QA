...
конкретные фреймворки, не унаследованный от какого-то специфического класса и 
не реализующий никаких служебных интерфейсов сверх тех, которые нужны для бизнес-модели.
То есть это такой класс, который не связан какими-либо особыми ограничениями, кроме тех, что налагаются спецификацией **Java**.

У **POJO** нет соглашения об именовании для свойств и методов.
...
**Простой Java Объект (Plain Old Java Object (POJO))** легко писать и понимать.
...
![Структура паттерна Простой Java Объект (Plain Old Java Object (POJO))](./_Files/1.%20Patterns/02.jpg "Структура паттерна Простой Java Объект (Plain Old Java Object (POJO))")

Класс POJO будет содержать только конструктор по умолчанию, частные члены данных и общедоступные методы установки и получения для всех членов данных.

1. *Поля класса*

Поля не ограничены модификаторами доступа.
У них могут быть любые модификаторы доступа:
...
В классе **Простой Java Объект (Plain Old Java Object (POJO))** нет необходимости добавлять конструктор.
...
Методы установки используются для установки значения переменной.

4. *Геттеры*

Методы получения используются для получения значения из переменной.

**Простой Java Объект (Plain Old Java Object (POJO))** не должен:

* расширять заранее заданные классы

Например: открытый класс, который расширяет **javax.servlet.http.HttpServlet** не является **Простым Java Объектом (Plain Old Java Object (POJO))**.

* реализовывать заранее заданные интерфейсы 

Например: открытый класс, который реализует **javax.ejb.EntityBean** не является **Простым Java Объектом (Plain Old Java Object (POJO))**.

* содержать предварительно определенные аннотации

Например: открытый класс, который содержит аннотацию **@ javax.persistence.Entity** не является **Простым Java Объектом (Plain Old Java Object (POJO))**.
...
**Простой Java Объект (Plain Old Java Object (POJO))** отделяет компоненты приложения от инфраструктуры.

3. *Упрощенное тестирование*

**Простой Java Объект (Plain Old Java Object (POJO))** можно протестировать или запустить вне сервера приложений.

4. *Гибкость*

**Простой Java Объект (Plain Old Java Object (POJO))** может быть реализован с любым типом корпоративного компонента, таким как объектный компонент, управляемый сообщениями, и объектный компонент. 
...
**Java боб (JavaBean)** - класс в языке **Java**, написанный по определённым правилам:
...
JavaBean-это особый тип POJO со строгим набором соглашений
...
## 2.4. Паттерн (Data Transfer Object)
