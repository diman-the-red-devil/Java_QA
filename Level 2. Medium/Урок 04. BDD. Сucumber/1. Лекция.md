...
исполняемый класс **Java**, который можно запустить из командной строки.
...
По умолчанию **Cucumber** будет рассматривать все файлы с расширением **.java**, как файл определения шага.
...
По умолчанию тесты запускаются в один поток.

*Пример*

```shell
java io.cucumber.core.cli.Main --threads 10 ./src/tests/resources/features
```

* *-g, --glue PATH* - пакет **Java**         

**PATH** - пакет **Java** с кодом шагов, хуков и т д.

Если пакет не указан, то **Cucumber** будет искать в classpath.
  
*Пример*

```shell
java io.cucumber.core.cli.Main --glue com.example.app ./src/tests/resources/features
```

* *-p, --plugin PLUGIN[:[PATH|[URI [OPTIONS]]]* - подключение плагинов

**PLUGIN** - наименование/тип плагина.

Встроенные плагинов форматов: html, json, junit, message, pretty, progress, rerun, teamcity, testng, timeline, usage.
Встроенные плагинов описания: summary, null_summary, unused.
...
```shell
java io.cucumber.core.cli.Main --plugin pretty ./src/tests/resources/features
```

* *-t, --tags TAG_EXPRESSION* - запуск по тегам сценариев             

**TAG_EXPRESSION** - список тегов.

Запуск только тех сценариев с тегами, что соответствующими **TAG_EXPRESSION**.

*Пример*

```shell
java io.cucumber.core.cli.Main --tags @dev @smoke ./src/tests/resources/features
```

* *-n, --name REGEXP* - запуск по наименованию сценариев                       

**REGEXP** - регулярное выражение.

Запуск только тех сценариев, чьи имена соответствуют **REGEXP**.

*Пример*

```shell
java io.cucumber.core.cli.Main --name .покупка* ./src/tests/resources/features
```

* *-d, --[no-]dry-run* - пропуск выполнения кода.                     

**--dry-run** - пропуск выполнения кода.

**--no-dry-run** - выполнение кода.

*Пример*

```shell
java io.cucumber.core.cli.Main --dry-run ./src/tests/resources/features
```

* *-m, --[no-]monochrome* - черно-белый вывод                   

**--monochrome** - черно-белый вывод терминала.

Вывод терминала не будет раскрашен. 

**--no-monochrome** - цветной вывод терминала.

Вывод терминала будет раскрашен в цвета принятые для статуса шагов в **Cucumber**.

*Пример*

```shell
java io.cucumber.core.cli.Main --monochrome./src/tests/resources/features
```

* *--snippets [underscore|camelcase]* - наименование для сгенерированных методов 

Соглашение об именах для сгенерированных методов. 

**underscore** - snake case.

Сгенерированные методы будут именоваться в "змеином" стиле (подчеркивание).

**camelcase** - camel case.

Сгенерированные методы будут именоваться в "верблюжьем стиле" (заглавные буквы).

По умолчанию - underscore (подчеркивание).
...
* *--i18n LANG* - список ключевых слов для определенного языка

**LANG** - язык

**--i18n help** - вывод списка всех языков.
...
Run the scenarios in a different order. 

The options are 'reverse' and 'random'. 
In case of 'random' order an optional seed parameter can be added 'random:<seed>'.
...
Number of scenarios to be executed. 
...
Uses the class specified by CLASSNAME as object factory. 
Be aware that the class is loaded through a service loader and therefore also needs to be specified in: 
...
```shell
java io.cucumber.core.cli.Main src/tests/resources/features/Smartphones.feature
```
...
```shell
java io.cucumber.core.cli.Main src/tests/resources/features/Smartphones.feature:3:9
```
...
Командную строку Cucumber можно использовать для быстрого запуска определенных тестов.
Он также поддерживает выполнение подмножества сценариев путем фильтрации тегов.

$ cucumber --tags @tag-name

Вышеупомянутая команда помогает в выполнении только тех сценариев, которые имеют указанное @ tag-name.
Аргументы могут быть предоставлены как логическая операция ИЛИ или И над тегами. 
Помимо тегов, сценарии можно фильтровать по названиям сценариев.

$ cucumber --name logout


Также полезно знать, что пошло не так, если тест не прошел.
Cucumber позволяет легко обнаруживать ошибки в коде с помощью опции --backtrace.

Cucumber также можно настроить так, чтобы игнорировать некоторые незавершенные сценарии, пометив их тегом «Работа в процессе» @wip.
Когда Cucumber передается аргумент --wip, Cucumber игнорирует сценарии с тегом @wip.
...
CLI
The Main class in the io.cucumber.core.cli package is used to execute the feature files.
You can run this class directly from the command line; in that case, there is no need to create any runner class.
The usage options for this class are mentioned here.
...
Add the two feature files (scenarios.feature and scenario-outlines.feature)
and step definition class as described in the JUnit section.

Open up a terminal window and navigate to the source folder of the project, in this case parallel.

Compile the step definition class.
Add the path to the folder containing cucumber jars to the classpath using the -cp option.
...
# 1. Формирование отчетов (Reports)
...
## 1.1. Встроенные плагины отчетов (Built-in Reporter Plugins)
...
## 1.2. Кастомные Форматтеры (Custom formatters)
