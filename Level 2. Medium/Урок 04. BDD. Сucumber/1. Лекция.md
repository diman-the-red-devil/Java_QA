...
Шаг ***Когда (When)*** проверяет результирующее состояние системы после выполнения действий. 
...
***И (And)*** - ключевое слово, предназначенное для связки.
...
***Но (But)*** - ключевое слово, предназначенное для связки.
...
Logically the same as And, but used in the negative form[
