...
| Тип    | Описание                                                   | document.readyState |
|--------|------------------------------------------------------------|---------------------|
| NORMAL | Загрузка всей страницы (по умолчанию)                      | complete            |
| EAGER  | Загрузка только HTML части (без стилей, изображений и т д) | interactive         |
...
пока не будет возвращено событие загрузки **document.DOMContentLoaded**.

То есть **Selenium WebDriver** будет ждать, пока исходный **HTML** документ не будет полностью загружен 
