...
***InvalidElementStateException*** исключение, которое возникает, 
если команда не может быть завершена из-за недопустимого элемента.
...
***InvalidCoordinatesException*** — исключение, которое возникает, 
когда координаты, передаваемые в команду, недействительны.
...
***MoveTargetOutOfBoundsException*** — исключение, которое возникает, 
если в метод **moveToElement(WebElement toElement)** передан элемент, находящийся за пределами экрана, 
...
***ElementNotSelectableException*** — исключение, которое возникает, 
когда элемент присутствует в **DOM**, но не может быть выбран.
