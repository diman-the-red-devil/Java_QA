...
* протокол передачи данных (**JSON Wire Protocol Over HTTP Client**) 

**JSON Wire Protocol** - REST API, который передает информацию между клиентом и сервером по **HTTP**. 
Каждый драйвер браузера (например, FirefoxDriver, ChromeDriver и т. Д.) имеет свой собственный HTTP-сервер.
...
2. Скрипт по протоколу **JSON Wire Protocol** посылает запросы по **HTTP** драйверу браузера **Browser Driver** с указанием действия, 
...
5. Скрипт по протоколу **JSON Wire Protocol** получает ответы по **HTTP** от драйвера браузера **Browser Driver** (полученные от реального браузера). 
